...
def private_key_to_public_key(private_key):
	...
	return f'04{pk.publicKey().toString().hex().upper()}'
...
def public_key_to_address(public_key):
	...
	var_encoded = f'00{var.hexdigest()}'.encode()
	digest = hashlib.sha256(binascii.unhexlify(var_encoded)).digest()
	var_hex = f'00{var.hexdigest()}' + hashlib.sha256(digest).hexdigest()[:8]
...
def private_key_to_WIF(private_key):
	...
	"""
	digest = hashlib.sha256(binascii.unhexlify(f'80{private_key}')).hexdigest()
